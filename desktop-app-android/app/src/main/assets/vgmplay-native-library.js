  ...
  class NativeLibraryApp {
    ...
    mount() {
      ...
      this.root.innerHTML = `
        ...
        <div class="native-mobile-tabs" data-role="mobile-tabs">
          <button data-mobile-tab="library" type="button">Library</button>
          <button data-mobile-tab="info" type="button">Info</button>
          <button data-mobile-tab="player" type="button">Player</button>
        </div>
        <div class="native-main">
          ...
          <div class="native-mobile-player-panel" data-role="mobile-player-panel">
            <div class="native-mobile-player-card">
              <div class="native-section-title">Now Playing</div>
              <div class="native-mobile-now-title" data-role="mobile-now-title">No track selected</div>
              <div class="native-mobile-now-source" data-role="mobile-now-source"></div>
              <div class="native-mobile-now-status" data-role="mobile-now-status">Idle</div>
              <div class="native-mobile-player-actions">
                <button data-role="mobile-bass" type="button">Bass</button>
                <button data-role="mobile-reverb" type="button">Reverb</button>
                <button data-role="mobile-random" type="button">Random</button>
                <button data-role="mobile-loop" type="button">Loop</button>
              </div>
              <label class="native-mobile-volume-row">Volume <input data-role="mobile-volume" type="range" min="0" max="100" value="80" /></label>
            </div>
          </div>
        </div>
      ...
      this.mobileNowSourceEl = this.root.querySelector('[data-role="mobile-now-source"]');
      this.mobileNowStatusEl = this.root.querySelector('[data-role="mobile-now-status"]');
      this.mobileBassBtn = this.root.querySelector('[data-role="mobile-bass"]');
      this.mobileReverbBtn = this.root.querySelector('[data-role="mobile-reverb"]');
      ...
      this.sortArchiveContentsEl.addEventListener('change', () => {
        this.config.sortArchiveContents = !!this.sortArchiveContentsEl.checked;
        this.saveConfig();
        this.renderTree();
      });
      ...
      if (this.volumeEl) {
        ...
        this.volumeEl.addEventListener('input', () => {
          ...
          if (this.mobileVolumeEl) this.mobileVolumeEl.value = String(vol);
          this.applyVolume();
          this.saveConfig();
        });
        this.applyVolume();
      }
      if (this.mobileVolumeEl) {
        this.mobileVolumeEl.value = this.config.volume != null ? this.config.volume : 80;
        this.mobileVolumeEl.addEventListener('input', () => {
          const vol = Number(this.mobileVolumeEl.value) || 0;
          this.config.volume = vol;
          if (this.volumeEl) this.volumeEl.value = String(vol);
          this.applyVolume();
          this.saveConfig();
        });
      }
      this.mobileTabButtons.forEach((button) => {
        button.addEventListener('click', () => this.setMobileTab(button.dataset.mobileTab || 'library'));
      });
      if (this.mobileBassBtn) this.mobileBassBtn.addEventListener('click', () => this.toggleBass());
    ...
    setMobileTab(tab) {
      const next = ['library', 'info', 'player'].includes(tab) ? tab : 'library';
      this.activeMobileTab = next;
      if (this.root) this.root.dataset.mobileTab = next;
    ...
    maybeShowInfoTab(entry) {
      if (!this.isPhoneLayout() || !entry) return;
      if (!entry.playable && !this.hasChildren(entry)) this.setMobileTab('info');
    }

    syncMobilePlayerPanel() {
      if (this.mobileNowTitleEl && this.nowTitleEl) this.mobileNowTitleEl.textContent = this.nowTitleEl.textContent || 'No track selected';
      if (this.mobileNowSourceEl && this.nowSourceEl) this.mobileNowSourceEl.textContent = this.nowSourceEl.textContent || '';
    ...
    visibleChildren(parentId) {
      ...
      if (this.shouldPreserveChildOrder(parentId)) return list;
      const rank = { folder: 0, archive: 1, pack: 1, archiveGame: 2, archiveTrack: 3, trackPart: 3, track: 4, unsupported: 5 };
    ...
    shouldPreserveChildOrder(parentId) {
      if (this.config.sortArchiveContents) return false;
      const parent = this.byId.get(parentId);
      if (!parent) return false;
      if (isArchiveEntry(parent) || parent.type === 'archiveGame') return true;
      const children = this.children.get(parentId) || [];
      return children.some((child) => child.type === 'trackPart');
    }
    ...
    renderRow(entry, depth, container) {
      ...
      row.addEventListener('click', () => this.activateRow(entry));
      row.addEventListener('dblclick', () => {
        if (!entry.playable) this.open(entry);
      });
    ...
    activateRow(entry) {
      if (this.isAncestorInspecting(entry)) return;
      const playableLeaf = entry && entry.playable && !this.hasChildren(entry);
      if (playableLeaf) {
        this.selectedId = entry.id;
        this.showInfo(entry);
        this.renderTree();
    ...
    showHelp() {
      this.infoMode = 'help';
      this.infoEl.innerHTML = '<div class="native-help"><strong>Welcome to VGMPlay-JS</strong><br>Open a local folder, hover entries to inspect metadata, tap playable tracks to load and play, and use search to filter by path or format.<br><br>Keyboard: Space = play/pause, Arrow keys = prev/next, Escape = back.</div>';
    }
    ...
    async buildVigamupMetadata(entry, entries, fileDataByPath, root, archiveTitle, archiveBase, sha) {
      ...
      const allTracks = [];
      let packCover = null;
      const orderedKssFiles = this.config.sortArchiveContents ? Array.from(kssFiles).sort((a, b) => a.localeCompare(b)) : kssFiles;
      for (const rel of orderedKssFiles) {
        const data = fileDataByPath.get(rel);
        const stem = baseName(rel).replace(/\.[^.]+$/, '').toLowerCase();
        const fsPath = `${root}/${rel}`.replace(/[\\]+/g, '/');
        this.writeBytesToFs(fsPath, data);
