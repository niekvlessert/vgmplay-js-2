  ...
  VGMPlay_js.prototype._cacheBridgeRequestAsync = async function (action, payload = {}) {
	...
	VGMPlay_js.prototype._ensureFileLoaded = async function (filepath, showLoading = true) {
		...
		if (showLoading) {
			...
			// Check if file belongs to a large archive
			let isLargeArchive = false;
			if (this.games && this.games.length) {
				for (const g of this.games) {
					if (g && g.files && g.archiveName) {
						for (const f of g.files) {
							if (f && f.filepath === filepath) {
								let totalSize = 0;
								for (const gf of g.files) {
									if (gf && gf.filepath && this._cacheFileSizes) {
										totalSize += this._cacheFileSizes[gf.filepath] || 0;
									}
								}
					...
					if (isLargeArchive) break;
				}
			}
			const msg = (size && size > threshold) || isLargeArchive
	...
	VGMPlay_js.prototype._restoreCacheFromBridge = async function () {
		...
		this._log && this._log('CACHE', '_cacheFileSizes entries:', Object.keys(this._cacheFileSizes || {}).length);

		// Calculate total size per archive
		const archiveSizes = new Map();
		if (this.games && this.games.length) {
			for (const g of this.games) {
				if (!g || !g.files || !g.archiveName) continue;
				let total = 0;
				for (const f of g.files) {
					if (f && f.filepath) {
						const size = this._cacheFileSizes ? this._cacheFileSizes[f.filepath] : 0;
						if (size) total += size;
					}
				}
				archiveSizes.set(g.archiveName, total);
				this._log && this._log('CACHE', 'Archive:', g.archiveName, 'size:', (total / 1024 / 1024).toFixed(2), 'MB', 'files:', g.files.length);
			}
		}
		...
		if (this.games && this.games.length) {
			for (const g of this.games) {
				...
				const archiveSize = g.archiveName ? (archiveSizes.get(g.archiveName) || 0) : 0;
				const isLargeArchive = archiveSize > largeArchiveThreshold;
				for (const f of g.files) {
					if (f && f.filepath) {
						const size = this._cacheFileSizes ? this._cacheFileSizes[f.filepath] : null;
						// Defer if: archive is larger than threshold OR (more than 50 games AND different host)
						const shouldDefer = isLargeArchive || (deferDifferentHost && !sameHost);
						if (shouldDefer) {
							largeFilePaths.push(f.filepath);
							continue;
						}
						// For current host with small archives, prefetch unknown sizes to restore track lists quickly.
						if (size != null && size > largeArchiveThreshold) {
							largeFilePaths.push(f.filepath);
						} else {
		...
		if (largeFilePaths.length) {
			this._log && this._log('CACHE', 'Deferring large files (>10MB archives or >50 total games from different host):', largeFilePaths.length);
		}
